...
public class JSAP {
	...
	static {
		if (Boolean.valueOf(System.getProperty("ahs.scratch.jsap.usenbsp", "false")).booleanValue()) {
			SYNTAX_SPACECHAR = '\u00a0';
		}
	}
	...
	public String getHelp(int screenWidth, String paramSeparator, boolean hideUnflagged) {
		...
		if (result == null) {
			...
			for (Iterator<Parameter> $itr = paramsByDeclarationOrder.iterator(); $itr.hasNext();) {
				...
				Iterator<String> helpInfo = us.exultant.ahs.util.Strings.wrapToList(param.getHelp() + defaultText, wrapWidth).iterator();
