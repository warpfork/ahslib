 ...
 * (semaphores, events, etc) that rely on first-in-first-out (FIFO) wait queues. This
 * class is designed to be a useful basis for most kinds of synchronizers that rely on a
 * single atomic <tt>int</tt> value to represent state. Subclasses must define the
 * protected methods that change this state, and which define what that state means in
 * terms of this object being acquired or released. Given these, the other methods in this
 * class carry out all queuing and blocking mechanics. Subclasses can maintain other state
 * fields, but only the atomically updated <tt>int</tt> value manipulated using methods
 * {@link #getState}, {@link #setState} and {@link #compareAndSetState} is tracked with
 * respect to synchronization.
 * 
 * <p>
 * Subclasses should be defined as non-public internal helper classes that are used to
 * implement the synchronization properties of their enclosing class. Class
 * <tt>AbstractQueuedSynchronizer</tt> does not implement any synchronization interface.
 ...
 * <em>shared</em> mode. When acquired in exclusive mode, attempted acquires by other
 * threads cannot succeed. Shared mode acquires by multiple threads may (but need not)
 * succeed. This class does not &quot;understand&quot; these differences except in the
 * mechanical sense that when a shared mode acquire succeeds, the next waiting thread (if
 * one exists) must also determine whether it can acquire as well. Threads waiting in the
 * different modes share the same FIFO queue. Usually, implementation subclasses support
 * only one of these modes, but both can come into play for example in a
 * {@link ReadWriteLock}. Subclasses that support only exclusive or only shared modes need
 * not define the methods supporting the unused mode.
 * 
 * <p>
 * This class defines a nested {@link ConditionObject} class that can be used as a
 * {@link Condition} implementation by subclasses supporting exclusive mode for which
 * method {@link #isHeldExclusively} reports whether synchronization is exclusively held
 * with respect to the current thread, method {@link #release} invoked with the current
 * {@link #getState} value fully releases this object, and {@link #acquire}, given this
 * saved state value, eventually restores this object to its previous acquired state. No
 * <tt>AbstractQueuedSynchronizer</tt> method otherwise creates such a condition, so if
 * this constraint cannot be met, do not use it. The behavior of {@link ConditionObject}
 ...
 * internal queue, as well as similar methods for condition objects. These can be exported
 * as desired into classes using an <tt>AbstractQueuedSynchronizer</tt> for their
 * synchronization mechanics.
 * 
 * <p>
 * Serialization of this class stores only the underlying atomic integer maintaining
 * state, so deserialized objects have empty thread queues. Typical subclasses requiring
 * serializability will define a <tt>readObject</tt> method that restores this to a known
 * initial state upon deserialization.
 * 
 * <h3>Usage</h3>
 * 
 * <p>
 * To use this class as the basis of a synchronizer, redefine the following methods, as
 * applicable, by inspecting and/or modifying the synchronization state using
 * {@link #getState}, {@link #setState} and/or {@link #compareAndSetState}:
 * 
 * <ul>
 * <li> {@link #tryAcquire}
 * <li> {@link #tryRelease}
 ...
 * Implementations of these methods must be internally thread-safe, and should in general
 * be short and not block. Defining these methods is the <em>only</em> supported means of
 * using this class. All other methods are declared <tt>final</tt> because they cannot be
 * independently varied.
 * 
 * <p>
 * You may also find the inherited methods from {@link AbstractOwnableSynchronizer} useful
 * to keep track of the thread owning an exclusive synchronizer. You are encouraged to use
 * them -- this enables monitoring and diagnostic tools to assist users in determining
 * which threads hold locks.
 * 
 * <p>
 * Even though this class is based on an internal FIFO queue, it does not automatically
 * enforce FIFO acquisition policies. The core of exclusive synchronization takes the
 * form:
 * 
 * <pre>
 * Acquire:
 ...
 * 
 * <p>
 * <a name="barging">Because checks in acquire are invoked before enqueuing, a newly
 * acquiring thread may <em>barge</em> ahead of others that are blocked and queued.
 * However, you can, if desired, define <tt>tryAcquire</tt> and/or
 * <tt>tryAcquireShared</tt> to disable barging by internally invoking one or more of the
 * inspection methods, thereby providing a <em>fair</em> FIFO acquisition order. In
 * particular, most fair synchronizers can define <tt>tryAcquire</tt> to return
 * <tt>false</tt> if {@link #hasQueuedPredecessors} (a method specifically designed to be
 ...
 * <em>greedy</em>, <em>renouncement</em>, and <em>convoy-avoidance</em>) strategy. While
 * this is not guaranteed to be fair or starvation-free, earlier queued threads are
 * allowed to recontend before later queued threads, and each recontention has an unbiased
 * chance to succeed against incoming threads. Also, while acquires do not
 * &quot;spin&quot; in the usual sense, they may perform multiple invocations of
 * <tt>tryAcquire</tt> interspersed with other computations before blocking. This gives
 * most of the benefits of spins when exclusive synchronization is only briefly held,
 * without most of the liabilities when it isn't. If so desired, you can augment this by
 * preceding calls to acquire methods with "fast-path" checks, possibly prechecking
 * {@link #hasContended} and/or {@link #hasQueuedThreads} to only do so if the
 * synchronizer is likely not to be contended.
 * 
 * <p>
 * This class provides an efficient and scalable basis for synchronization in part by
 * specializing its range of use to synchronizers that can rely on <tt>int</tt> state,
 * acquire, and release parameters, and an internal FIFO wait queue. When this does not
 ...
 * represent the unlocked state, and one to represent the locked state. While a
 * non-reentrant lock does not strictly require recording of the current owner thread,
 * this class does so anyway to make usage easier to monitor. It also supports conditions
 ...
 * 
 * <p>
 * Here is a latch class that is like a {@link CountDownLatch} except that it only
 * requires a single <tt>signal</tt> to fire. Because a latch is non-exclusive, it uses
 * the <tt>shared</tt> acquire and release methods.
 * 
 * <pre>
 * class BooleanLatch {
 * 	
...
public abstract class AQS extends AbstractOwnableSynchronizer {
	 ...
	 * thread. The status field does NOT control whether threads are granted locks etc
	 ...
	 * of their predecessors. This is solved when necessary by checking backwards from
	...
	static final class Node {
		 ...
		 * acquire methods must first indicate they need a signal, then retry the
		 * atomic acquire, and then, on failure, block. CANCELLED: This node is
		 * cancelled due to timeout or interrupt. Nodes never leave this state. In
		 * particular, a thread with cancelled node never again blocks. CONDITION:
		 * This node is currently on a condition queue. It will not be used as a
		 * sync queue node until transferred, at which time the status will be set
		 * to 0. (Use of this value here has nothing to do with the other uses of
		 * the field, but simplifies mechanics.) PROPAGATE: A releaseShared should
		 ...
		 * of GC) only upon dequeuing. Also, upon cancellation of a predecessor,
		 * we short-circuit while finding a non-cancelled one, which will always
		 * exist because the head node is never cancelled: A node becomes head
		 * only as a result of successful acquire. A cancelled thread never
		 * succeeds in acquiring, and a thread only cancels itself, not any other
		 * node.
		 */
		volatile Node		prev;
		 ...
		 * node is at end of queue. However, if a next field appears to be null,
		 * we can scan prev's from the tail to double-check. The next field of
	 ...
	 * invoking at least once {@link #tryAcquire}, returning on success. Otherwise,
	 ...
	 * thread is attempting to acquire in shared mode (that is, this method is invoked
	 * from {@link #tryAcquireShared}) then it is guaranteed that the current thread
	 ...
	 * {@link #tryAcquire} method should return {@code false}, and its
	 * {@link #tryAcquireShared} method should return a negative value, if this method
	...
	public class ConditionObject implements Condition, java.io.Serializable {
		 ...
		 * full traversal, it comes into play only when timeouts or cancellations
		 * occur in the absence of signals. It traverses all nodes rather than
		 * stopping at a particular target to unlink all pointers to garbage nodes
